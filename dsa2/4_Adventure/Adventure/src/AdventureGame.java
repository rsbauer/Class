...
public class AdventureGame extends JPanel implements KeyListener
{
	...
	/**
	 * Keep track of which room will be the starting room (this allows the room to appear anywhere 
	 * in the castle.dat file or get renamed to something new. 
	 */
	private static final String STARTROOM = "Gatehouse";  // "Entrance Hall";
	
	/**
	 * Keep track of the numeric select mode we are currently in.  This may be to specify if the 
	 * player is selecting an item to pick up, remove, or use. 
	 * 
	 * 0 = do nothing
	 * 1 = take
	 * 2 = remove
	...
	/**
	 * Identify the start of the game - this allows the check in roomInfo() to not kill the player 
	...
	private static final String WELCOMEMSG = "Welcome to Adventure!\n\nFor " +
			"help, press H\n\nThe king has requested your help.  You must retrieve his crown " +
			"from the castle.  Only return to the entrance hallway when you have the crown.  " +
			"Watch out for treasures and evil-doers.\n";  
	...
	private void setupPanel()
	{
		...
		// setup scroll bars for the editor pane
		JScrollPane editorScrollPane = new JScrollPane(editorPane);		
		// turn on vert. scrollbar
        ...
        // set to horizontal to align center and fill both sides
        ...
        /*
         * Had problems setting up a JTextPane (be it JTextPane, JTextArea, or JEditorPane) to 
         * receive focus using [object].requestFocus().  Found a comment here about the problem: 
         ...
         * http://java.sun.com/j2se/1.4.2/docs/api/java/lang/Runnable.html and setup the run() 
         * method.  Success!
         * 
         * The purpose of all of this is to direct the focus to the editor so the keyboard listener 
         * could start and the user's commands would be processed.   
         */
        SwingUtilities.invokeLater(new Runnable() 	// setup another thread to run some commands
	        { 
		        // code to run:  set the focus back to the editor
	        	public void run() { editorPane.requestFocus(); }	
	        });												
...
//        editorPane.setContentType("text/html"); // text/html
	...
	/**
	 * Load the specified file and parse it.  While parsing, populate a graph.  The graph will 
	 * represent the game's rooms.  Each room will contain an array of edges which will also be 
	 * determined from the specified file.
	 * 
	 * @param filename
	 */
    			...
    			if(!line.equals("99"))
    			{
			    	switch(task)
			    	{
			    		case 0:								// room number and room name
			    			...
			    			// add a room
			    			room = castle.addRoom(Integer.parseInt(field[0]) - 1, field[1]);			
			    		...
			    		case 1:
			    			...
			    			// add the arc
			    			castle.addArc(field[2], Integer.parseInt(field[0]) - 1, 
			    				Integer.parseInt(field[1]) - 1);	
			    			break;
	...
	/**
	 * Monitor for key typed.  This actually is looking for special keys like the Window key and 
	 * such.
	 * 
	...
	/**
	 * Handles key presses.  Upon receiving notice of a key press, figure out which key was 
	 * pressed and take action if necessary.
	 * 
	...
	public void keyPressed(KeyEvent e) 
	{
		...
		if(key > 64 && key < 91)							// key is in A-Z (uppercase) range
		{
			...
			switch(key)										// Monitor the integer ASCII keyed value
			{
				...
				case 85:									// U - use an item
					selectUseItem();						// find out item the player wants to use
					showCursor();
					break;
		...
		// key is between 0-9 and user is to select an item
		if(key > 47 && key < 58 && selectmode > 0)					
		{
			switch(selectmode)									// switch based on the mode we're in
			{
				case 1:											// selecting an item to take
					// grab the room item (subtract 48 to convert the ASCII value to the integer 
					// value					
					takeRoomItem(key - 48);							
					break;
					
				case 2:
					// remove the desired item from the backpack and place back into the current 
					// room
				...
				case 4:										// player is giving something to the hag
					...
					if(key - 48 == 0)			// check if the user selected "0 - Nothing" to give
					{
						...
						printMessage("You didn't give anything to the hag?  She beats you severely " +
								"with a big stick.\n");
					}  // end if(key - 48 == 0)
					else
					{
						...
						printMessage("The old hag takes your " + packitem.fullname.toLowerCase() 
								+ " and spares you a beating.\n");
					}  // end else
	...
	/**
	 * Displays current room info as well as the cursor.  Also performs a check if the user 
	 * returned to the entrance hall and determines if they won the game or not. 
	 */
	public void roomInfo()
	{
		...
		else
		{
			if(player.getHealth() <= 0)
				printMessage("\n\nYour health has failed you and you've failed to return the " +
						"crown.  You have died without honor.\n\nG A M E   O V E R");
			else
			{
				// let the user know what room they're in and what's in it
				printMessage(currentRoomInfo());			
				showCursor();
			}  // end else 
		}  // end else
		
		// update the status bar with the players health
		statusbarHealth.setText("Health: " + player.getHealth() + "%");	
		statusbarRoom.setText("Room: " + player.getCurrentRoom().getName());
	}  // end public void startGame()
	

	/**
	 * Get the current room and format the the information to let the user know where they are and 
	...
	private String currentRoomInfo()
	{
		...
		{										// handy for showing the user where the doors are 
			...
			output.append(castle.getDirectionFullName(((Arc) arcs.get(a)).getLabel()));	
		...
		// assemble the string buffer into a string & return it
	...
	/**
	 * Make a move.  The direction specifies the direction.  Make sure this is a valid direction 
	 * the player can go in.  If it is, set the player's current room to the new room.
	 *  
	...
	private void makeMove(char direction)
	{
		...
		if(newroom != null)
		{
			/* game now in play - trick here is the player may select the wrong door and bounce 
			back to the entrance hall without having left.  In this case, don't kill the player yet.
			*/ 
			gamestart = false;
	...
	/**
	 * The player has requested to flee.  Start here and find the shortest path.  Then display the 
	 * path to the player. 
	 */
	private void flee()
	{
		ArrayList<DSPRoomSet> fleepath = castle.shortestPath(player.getCurrentRoom(), 
				castle.getEntrance());
	...
	/**
	 * Convert the fleepath array list to a string suitable for output to the user.  Work backwords 
	 * on the fleepath array list.  By doing so, we can traverse the array from the entrance way 
	 * and find matching rooms which will adventually lead back to our current room.  Trick is to 
	 * build the output string just right so the rooms appear in logical order (display the current 
	 * room first, then the next room is the room the user would go to next to make their way to 
	 * the entrance).
	 * 
	 * @param fleepath the array list containing the flee path to take.  It is an array of 
	 * DSPRoomSet objects.
	...
	private String fleepath2String(ArrayList<DSPRoomSet> fleepath)
	{
		...
		for(int a = fleepath.size() - 1; a >= 0; a--)
		{
			...
			{																	// nope!
				...
				}  // end if(lastRoom.getRoomID() == ((DSPRoomSet) fleepath.get(a))...
			}  // end if(((DSPRoomSet) fleepath.get(a)).getRoomConnects() != null)
		}  // end for(int a = fleepath.size() - 1; a >= 0; a--)
		
		if(output.equals(""))
		{
			output = "You are stuck in the " + player.getCurrentRoom().getName() + 
				".  There's no where to flee.";
		}  // end if(output.equals(""))
		else
	...
	public String listItems(Room room)
	{
		...
		for(int a = 0; a < items.size(); a++)
		{
			...
			// sounds funny to say "I found a nothing" so change it to "I found nothing" and drop 
			// the a
			if(!itemname.equals("nothing"))
				output.append("a");
			
			// check for a vowel so the "a" becomes "an"
			if(itemname.charAt(0) == 'a' || itemname.charAt(0) == 'e' || 
					itemname.charAt(0) == 'i' || itemname.charAt(0) == 'o' || 
					itemname.charAt(0) == 'u')
				output.append("n");
	...
	/**
	 * If possible, take a room item and place in the backpack.  If there's only one item and its 
	 * moveable, then automatically take it.  Otherwise, present a choice to the user and allow 
	 * them to select which item to take. 
	 */
	public void take()
	{		
...
//				printMessage((a + 1) + ". " + ((Item) moveable.get(a)).getName());
	...
	/**
	 * Take the the item the player selected and remove it from the room and add it to the player's 
	 * backpack (as long as the backpack has room).
	 * 
	...
	public void takeRoomItem(int item)
	{
		...
		if(player.backpackHasRoom() == true)
		{
			...
			printMessage("Your backpack is full!  (R)emove an item before adding additional " +
					"items.\n");
	...
	/**
	 * Remove an item from the player's backpack - first show a list of items available to be 
	 * removed. 
	 */
	 ...
	 * 
	 * @param index Index value of the item to remove from the backpack (its 1 based, will need to 
	...
	/**
	 * Allow the player to use an item.  Figure out which item the player selected, determine if 
	 * it exists in the backpack, then use it.     
	 * 
	 * @param index
	 */
	...
	/**
	 * Loop through all the items in the list and format the item name for output to the user as a 
	 * selection item.
	 * 
	...
	private void playCharacters(Room room)
	{
		...
		for(int a = 0; a < room.getCharacters().size(); a++)
		{
			...
			switch(character)
			{
				...
				case hag:
					// present the user the option of giving something to the hag.  If they don't, 
					// then take away some health points
					selectmode = 4;				// the backpack item selection mode is for the hag
					printMessage("The hag holds out her hand.  She's expecting you to give her " +
							"an item.\nSelect the item to give her:");
				...
				case werewolf:
					// the werewolf is still alive, check if the player used a spell.  
				...
				case hunchback:
					...
					{
						// randomly pick & remove item
						RoomItem stolen = player.randomRemoveBackpackItem();		
						// return the item to the prev room
						player.getPreviousRoom().addItem(stolen);					
						
						printMessage("\nUh oh!  The hunchback just swiped " 
								+ stolen.fullname.toLowerCase() + " from your pack.\n");
			...
			{
				// yup, get the characters health adder and add it
				player.addHealth(character.health);				
	...
	/**
	 * Check the item used and if the right item is used, remove (kill) the associated character 
	 * in the room, if it exists.
	 * 
		...
		switch(item)
		{
			...
			case woodenstake:
				if(player.getCurrentRoom().removeVampire())
					printMessage("Using your wooden stake you manage to drive it through the " +
							"vampire's heart.  You killed the vampire!\n");
				break;
		}  // end switch(item)
