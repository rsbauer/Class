...
public class Castle 
{
	...
	/**
	 * Set up a number that has a value higher than any possible weight to be calculated for this graph/castle 
	 */
	...
	private enum Direction
	{
		...
		{
			fullName = name;							// create the association of the letter to the name
...
//		adjacency = clearMatrix(new byte[roomcount][roomcount]);		// clear the matrix and set the values to 0
	...
	public Room testMove(int currentroom, char direction)
	{
		...
		for(int a = 0; a < arcs.size(); a++)
		{
			...
			// test if the room equals the current room and if the arc is going in the direction the user wanted
	...
	/**
	 * Using Dijkstra's shortest path algorithm, find the shortest path between the current room the user is in back
	 ...
	 * http://www.math.grin.edu/~rebelsky/Courses/CS152/98S/Outlines/outline.50.html</a>.  The algorithm built doesn't
	 * match the one's presented the class notes or the link.  The algorithm built uses an adjacency matrix to locate
	 * locate the neighbors and to update their distance from the starting node/room.  Before finding its neighbors,
	 * the shortest distance node/room is found and placed in the found list and removed from the search list.  With the
	 * new node, find its neighbors, update their weight/cost and return the updated hash map.  Keep repeating until
	 ...
	 */
	public ArrayList shortestPath(Room currentroom, Room destinationroom)
	{
		// keep track of we're done - false means the destination room is not in the found list (yet)
		boolean done = false;											
		
		
		ArrayList shortestpath = new ArrayList();									// keep track of the found nodes/rooms
		Map search = new HashMap();										// hash containing all rooms, at the start

		
		for(int a = 0; a < roomcount; a++)								// initialize the search map - load up the rooms	
		{
		...
		DSPRoomSet smallest = null;							// object to keep track of the smallest distance node/room
	...
	/**
	 * Locate a nodes neighbors.  Loop through the number of rooms.  Check the adjacency list for a 1.  If there's a one
	 * then the index value a represents an adjacent room.  Calculate the room's distance/weight.  Once updated, 
	 * update the search hash map with the new information.
	 * 
	 ...
	 */
	private Map findNeighbors(Map search, DSPRoomSet root)
	{
		for(int a = 0; a < roomcount; a++)									// loop all rooms
		{
			if(adjacency[root.getRoom().getRoomID()][a] == 1)				// check if adjacent
			{
				...
				if(search.containsKey(a))									// check if this key is still available
				{
					DSPRoomSet set = (DSPRoomSet) search.get(a);			// get the cost/room combo object
					Arc arc = findArc(root.getRoom(), set.getRoom());		// find the arc from root to this room

					// check if the previous node's cost + weight is less than the current cost
					if(root.getCost() + arc.getWeight() < set.getCost())	
					{														// yup, it is
						set.setCost(root.getCost() + arc.getWeight());		// update the cost/room combo object
						set.setRoomConnects(root.getRoom());				// update the roomConnects value
						set.setLabel(arc.getLabel());						// set the arc info
						search.put(a, set);									// update the search hash map
	 ...
	 * @return DSPRoomSet object containing a reference of the matching room and its cost/weight information
	 */
	...
	/**
	 * Add an item to a room.  The item MUST be in the enumerated RoomItems list.  If not, the item will not be added.
	 * 
