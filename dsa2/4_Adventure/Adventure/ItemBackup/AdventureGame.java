...
//		editorPane.setText(editorPane.getText() + sText + "\n" + CURSOR);
	...
	private void makeMove(char direction)
	{
		...
		if(newroom != null)
		{
			...
			// TODO:  check if there was a bad guy in the last room - update stats as necessary
			for(int a = 0; a < room.getItems().size(); a++)
			{
				if(((Item) room.getItems().get(a)).isCharacter() == true)
				{
					Item character = (Item) room.getItems().get(a);			// get the character
					player.addHealth(character.getHealthAdder());			// add/subtract player's health
						
					switch(character.getRoomItem())
					{
						case vampire:
					...
					}  // end switch(character.getRoomItem())
				
				}  // end if(((Item) room.getItems().get(a)).isCharacter() == true)
			}  // end for(int a = 0; a < room.getItems().size(); a++)
	...
	private void flee()
	{
		ArrayList fleepath = castle.shortestPath(player.getCurrentRoom(), castle.getEntrance());
	 ...
	 */
	private String fleepath2String(ArrayList fleepath)
	{
		...
		{
			if(((DSPRoomSet) fleepath.get(a)).getRoomConnects() != null)		// check if connecting room is null
			{																	// nope!
				// check if last room connects to this room
				if(lastRoom.getRoomID() == ((DSPRoomSet) fleepath.get(a)).getRoom().getRoomID())	
				{
					// get the room name
					name = (((DSPRoomSet) fleepath.get(a)).getRoom().getName());
					
					// get the direction's full name
					direction = castle.getDirectionFullName(((DSPRoomSet) fleepath.get(a)).getLabel());
					
					// build the output string
					output = ("\nWent " + direction + " to the " + name + ".").concat(output);
					
					// make a note of this room's connection and set it up as the last room
					lastRoom = ((DSPRoomSet) fleepath.get(a)).getRoomConnects();
	...
	{
		ArrayList items = room.getItems();							// list the items in the room
	...
	{		
		...
		ArrayList moveable = player.getCurrentRoom().getMoveableItems();
		if(moveable.size() > 0)
		{
...
//				printMessage((a + 1) + ". " + ((Item) moveable.get(a)).getName());
	...
	public void takeRoomItem(int item)
	{
		...
		{
			ArrayList moveable = player.getCurrentRoom().getMoveableItems();
			Item roomitem = (Item) moveable.get(item - 1);
			player.addBackpackItem(roomitem);
			player.getCurrentRoom().removeItem(roomitem);
			
			printMessage(roomitem.getName() + " added to your backpack.\n");
	...
	{
		ArrayList backpack = player.getBackpack();
		if(backpack.size() > 0)
		{
	...
	public void removeBackpackItem(int index)
	{
		...
		ArrayList backpack = player.getBackpack();						// get the backpack

		// check and make sure the item requested to be removed is in range
		if(index - 1 < backpack.size())
		{	
			Item backpackItem = (Item) backpack.get(index - 1);			// get the item to work with
			...
			printMessage(backpackItem.getName() + " removed.\n");
			printMessage(player.listContents());						// list contents
			roomInfo();
		}  // end
		else
	...
	public void useItem(int index)
	{
		...
		{	
			Item backpackItem = (Item) backpack.get(index - 1);			// get the item to work with
			player.removeBackpackItem(backpackItem);					// remove this item from the backpack
			player.addHealth(backpackItem.getHealthAdder());			// add/subtract player's health
			selectmode = 0;												// reset the mode
			
			// TODO:  Perform action with the item used!
			printMessage(backpackItem.getName() + " used.\n");
			printMessage(player.listContents());						// list contents
			roomInfo();
		}  // end
		else
	 ...
	 */
	public String displayList(ArrayList backpack)
	{
		StringBuffer output = new StringBuffer();
