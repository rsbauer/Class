...
{	
	...
	public DrawTree(String fp, FloorPlan fpobject)
	{
		floorplan = fp;
		floorplanbutton = new Command("Floor Plan", Command.SCREEN, 1);
		exit = new Command("Exit", Command.EXIT, 1);
		back = new Command("Back", Command.BACK, 1);
		newplan = new Command("New", Command.SCREEN, 1);

		addCommand(floorplanbutton);
		addCommand(newplan);
		addCommand(back);
		addCommand(exit);
...
//		g.drawRect(0, 0, 10, 10);
	...
	{
		if(command == floorplanbutton)
		{
			Display display = floorplanform.getDisplay();
			Canvas canvas = new DrawFloorPlan(floorplan, floorplanform);
			display.setCurrent(canvas);
		}  // end if(command == floorplanbutton)
		
		if(command == newplan || command == back)
		{
