...
public class BinaryTree 
{
	...
	private void labelNode(Graphics g, BinaryNode node, Point center, int boxsize)
	{
		...
		g.setColor(0, 0, 0);											// set the label to the line color
		...
		switch(character)													// determine the draw instruction to use
		{
			...
			default:														// dealing with a letter
				g.drawString("" + node.getData(), center.x - 2, center.y - 4, 0);		// label the node
				break;
	...
	private void drawFilledSquare(Graphics g, Point center, int boxsize)
	{
		...
		g.setColor(17, 185, 255);										// set the fill color
		g.fillRect(center.x - halfbox, center.y - halfbox, boxsize, boxsize);	// draw the box
		g.setColor(0, 0, 0);										// set the line color
	...
	private void drawFilledCircle(Graphics g, Point center, int boxsize)
	{
		...
		g.setColor(17, 185, 255);										// set the fill color
		g.fillArc(center.x - halfbox, center.y - halfbox, boxsize, boxsize, 0, 360);	// draw the circle
		g.setColor(0, 0, 0);										// set the line color
		g.drawArc(center.x - halfbox, center.y - halfbox, boxsize, boxsize, 0, 360);	// out the circle (makes it pretty!)
	...
	{
		g.setColor(0, 0, 0);								// set the line color (note, outer box already drawn)
