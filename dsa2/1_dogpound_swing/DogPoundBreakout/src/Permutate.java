...
public class Permutate extends Vector {
	 ...
	 */
	public Vector findMatches(String[] aName) {
		boolean bFound = false;												// flag to keep track if we found a name or not
		Vector vName = new Vector();										// setup the vector to hold the found names
		
		for(int c = 0; c < aName.length; c++) {								// loop all the test names
			bFound = false;

			// loop all the permutations of the jumble - stop looping if we find a match
			for(Enumeration e = this.elements(); e.hasMoreElements() && bFound == false;) {	

				// check if the test name can be found in any of the permutations
				if(e.nextElement().toString().indexOf(aName[c]) != -1)		
				
					if(!vName.contains(aName[c])) {						// found a match - now check if its been found before
						boolean b = vName.add(aName[c]);							// hasn't been found before - add it to the vector
						if(b == true)
							bFound = true;									// could have used break here instead - set the flag to true
